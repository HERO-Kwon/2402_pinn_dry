...
class Energy_layer(torch.nn.Module):
    def __init__(
            ...
            nx=21, ny=21, length_x = 0.1, length_y=0.1, bcs=None,
    ):
    ...
    def advection_diffusion(self, x):
        return self.u*self.Dx(x)/self.h+self.v*self.Dy(x)/self.h - self.diff_coeff*(self.laplace(x))/self.h/self.h
    def boundary(self,x):
    ...
    def forward(self, layout, heat, flow):
        ...
        # Source item
        f = self.cof * abs(geom-1)*30
        
        # The nodes which are not in boundary
        G_bc = torch.ones_like(heat).detach()
        G_bc[...,1:-1,1:-1] = 0
        G_bc_value = torch.zeros_like(heat).detach()

        indices_in = (boundary == 1).nonzero(as_tuple=True)
        heat[..., indices_in[0], indices_in[1]] = 0

        x = pad(heat, [1, 1, 1, 1], mode='reflect')  # constant, reflect, reflect
        
        loss_interior = self.advection_diffusion(x)[...,1:-1,1:-1]
        loss_boundary = G_bc * self.boundary(x)
        
        loss = loss_boundary + pad(loss_interior, [1,1,1,1], value=0)
        
        indices_1diff = (boundary > 3).nonzero(as_tuple=True)
        G_bc0 = geom.clone()
        G_bc0[...,:,indices_1diff[0],indices_1diff[1]] = 0
        G_bc0[...,:,indices_in[0],indices_in[1]] = 0
        indices_bc4 = (boundary == 4).nonzero(as_tuple=True)
        G_bc4 = torch.zeros_like(heat)
        G_bc4[...,:,indices_bc4[0],indices_bc4[1]] = 1
        indices_bc5 = (boundary == 5).nonzero(as_tuple=True)
        G_bc5 = torch.zeros_like(heat)
        G_bc5[...,:,indices_bc5[0],indices_bc5[1]] = 1
        indices_bc6 = (boundary == 6).nonzero(as_tuple=True)
        G_bc6 = torch.zeros_like(heat)
        G_bc6[...,:,indices_bc6[0],indices_bc6[1]] = 1
        indices_bc7 = (boundary == 7).nonzero(as_tuple=True)
        G_bc7 = torch.zeros_like(heat)
        G_bc7[...,:,indices_bc7[0],indices_bc7[1]] = 1
        indices_bc8 = (boundary == 8).nonzero(as_tuple=True)
        G_bc8 = torch.zeros_like(heat)
        G_bc8[...,:,indices_bc8[0],indices_bc8[1]] = 1
        indices_bc9 = (boundary == 9).nonzero(as_tuple=True)
        G_bc9 = torch.zeros_like(heat)
        G_bc9[...,:,indices_bc9[0],indices_bc9[1]] = 1
        indices_bc10 = (boundary == 10).nonzero(as_tuple=True)
        G_bc10 = torch.zeros_like(heat)
        G_bc10[...,:,indices_bc10[0],indices_bc10[1]] = 1
        indices_bc11 = (boundary == 11).nonzero(as_tuple=True)
        G_bc11 = torch.zeros_like(heat)
        G_bc11[...,:,indices_bc11[0],indices_bc11[1]] = 1
        
        '''
        4: BD x, 5: FD y, 6: FD x, 7: BD y
        8: 4+5, 9: 5+6, 10: 6+7, 11: 4+7 
        '''
        
        loss = G_bc0 * loss
        loss += G_bc4 * -30*self.BDx(x)
        loss += G_bc6 * -30*self.FDx(x)
        loss += G_bc7 * -30*self.BDy(x)
        loss += G_bc8 * -30*(self.BDx(x)+self.FDy(x))
        loss += G_bc9 * -30*(self.FDy(x)+self.FDx(x))
        loss += G_bc10 * -30*(self.FDx(x)+self.BDy(x))
        loss += G_bc11 * -30*(self.BDx(x)+self.BDy(x))
        
        return self.base_loss(loss - f, torch.zeros_like(loss))
