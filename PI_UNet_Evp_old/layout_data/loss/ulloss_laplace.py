...
class Energy_layer(torch.nn.Module):
    def __init__(
        ...
        self.h = self.length_x / (self.nx - 1)
        # ((l/(nx))^2)/(4*cof)*m*input(x, y)
        self.cof = -1 * self.h ** 2 / TEMPER_COEFFICIENT
    ...
    def forward(self, layout, heat):
        ...
        layout_pred[..., indices_in[0], indices_in[1]] = self.cof * geom[..., indices_in[0], indices_in[1]]
        
        indices_out = (boundary == 3).nonzero(as_tuple=True)
        layout_pred[..., indices_out[0], indices_out[1]] = self.cof * geom[..., indices_out[0], indices_out[1]]
        
        #print(layout_pred.shape)
        #print(layout.shape)
        return self.base_loss(layout_pred, self.cof * geom)
